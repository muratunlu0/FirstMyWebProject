$(document).ready(function(){
  ...
  var colors = ["#E15F59","#F9D458", "#5EBE86", "#4890E8"];
  ...
  function findNearby() {
    ...
    if (checkAuth() != false) {
      ...
      $.get('https://nominatim.openstreetmap.org/search?viewbox='+coordinates+'&format=geocodejson&limit=100&bounded=1&amenity='+locationtype+'&exclude_place_ids='+JSON.stringify(place_ids), function(data) {
  ...
  function saveNearby() {
    ...
    if (checkAuth() != false) {
      ...
      currentid = db.ref("markers").push().key;
      var key = currentid;
      inst.id = currentid;
      db.ref("markers/"+currentid).set({
        color: inst.color,
  ...
  function saveForm(e) {
    ...
    if (checkAuth() != false) {
      ...
      if (inst.type == "area") {
        ...
        db.ref("markers/"+currentid).update({
          area:inst.area,
  ...
  function cancelForm() {
    ...
    if (checkAuth() != false) {
      ...
      if (inst.type == "area") {
        ...
        //   // completed: true
        // })
        db.ref("markers/"+currentid).set(null)
      }
      ...
      workingLayer.on('pm:vertexadded', e => {
        if (e.shape == "Polygon") {
          ...
          if (e.layer._latlngs.length == 1) {
            ...
            db.ref("markers/"+currentid+"/coords/").push({
  ...
  function createMarker(lat, lng, user) {
    if (markerson) {
      ...
      marker.bindTooltip('<label for="shape-name">Name</label><input value="Konum" id="shape-name" name="shape-name" /><label for="shape-desc">Description</label><textarea id="shape-desc" name="description"></textarea><br><div id="buttons"><button class="cancel-button">Cancel</button><button class="save-button">Save</button></div><div class="arrow-down"></div>', {permanent: true, direction:"top", interactive:false, bubblingMouseEvents:false, className:"create-shape-flow create-form", offset: L.point({x: 0, y: -35})});
      marker.addTo(map);
      marker.openTooltip();
      ...
      objects.push({id:currentid, user:user.uid, color:color, name:"Konum", m_type:"none",  desc:"", lat:lat, lng:lng, marker:marker, trigger:marker, session:session, completed:true, type:"marker"});
      ...
      marker.on('tooltipclose', function(e){
        if (enteringdata) {
          ...
          // cancelForm();
          marker.remove();
          db.ref("markers/"+inst.id).set(null);
  ...
  function renderObjectLayer(object) {
    ...
    if ($(".annotation-item[data-id='"+object.id+"']").length == 0) {
      ...
      if (object.type == "line") {
        ...
        $("#annotations-list").prepend('<div class="annotation-item" data-id="'+object.id+'"><div class="annotation-name"><img class="annotation-arrow" src="assets/arrow.svg">'+icon+'<span>'+object.name+'</span><img class="delete-layer" src="assets/delete.svg"></div><div class="annotation-details"><div class="annotation-description">'+object.desc+'</div><div class="annotation-data"><div class="annotation-data-field"><img src="assets/marker-small-icon.svg">'+object.lat.toFixed(5)+', '+object.lng.toFixed(5)+'</div></div></div></div>');
      }
    } else {
  ...
  $(document).on("click", "#zoom-in", zoomfpiechartIn);
